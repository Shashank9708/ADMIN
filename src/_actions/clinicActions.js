import { configConstants, clinicConstants } from '../_constants';
...
function getClinicList(id) {
    return dispatch => {
        dispatch(request(id));
        return clinicService.getClinicList(id)
            .then(
                response => {
...
function getCityList(id) {
    return dispatch => {
        dispatch(request(id));
        return clinicService.getCityList(id)
            .then(
                response => {
                    if(response !== "Error: Network Error"){
                        var data = response.data;
...
// Actions defination that will perform according dispatch call and send data to reducer
    function request() { return { type: clinicConstants.CITY_REQUEST } }
    function success(result) { return { type: clinicConstants.CITY_SUCCESS, result } }
    function failure(error) { return { type: clinicConstants.CITY_FAILURE, error } }
    function unauthorize(error) { return { type: clinicConstants.UNAUTHENTICATE, error } }
    function serverDown(error) { return { type: configConstants.SERVER_DOWN, error } }
}
